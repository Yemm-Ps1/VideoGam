...
func test_getName_namesNotSpecifiedInConstructor_shouldMatchExpectedFormat():
	var agent = AgentClass.new(empty_grid)
	var expected_regex = RegEx.new()
	var found_agent_name : String = agent.get_agent_name()
	assert_true(found_agent_name.match("Agent:?*"), "Name found was invalid: " + found_agent_name)
